...
static int aspeed_video_start_frame(struct aspeed_video *video)
{
	...
	if (video->v4l2_input_status) {
		v4l2_dbg(2, debug, &video->v4l2_dev, "No signal; don't start frame\n");
		return 0;
	}

	if (!(seq_ctrl & VE_SEQ_CTRL_COMP_BUSY) ||
	    !(seq_ctrl & VE_SEQ_CTRL_CAP_BUSY)) {
		v4l2_dbg(2, debug, &video->v4l2_dev, "Engine busy; don't start frame\n");
		return -EBUSY;
	}
	...
	if (!buf) {
		...
		v4l2_dbg(2, debug, &video->v4l2_dev, "No buffers; don't start frame\n");
		return -EPROTO;
	}
...
static void aspeed_video_get_resolution(struct aspeed_video *video)
{
	...
	do {
		...
		rc = wait_event_interruptible_timeout(video->wait,
						      ...
						      MODE_DETECT_TIMEOUT);
		if (!rc) {
			v4l2_dbg(2, debug, &video->v4l2_dev, "Timed out; first mode detect\n");
		...
		clear_bit(VIDEO_RES_DETECT, &video->flags);
		if (!rc) {
			v4l2_dbg(2, debug, &video->v4l2_dev, "Timed out; second mode detect\n");
			return;
		}
	...
	if (invalid_resolution) {
		v4l2_dbg(2, debug, &video->v4l2_dev, "Invalid resolution detected\n");
		return;
	}
...
static void aspeed_video_stop_streaming(struct vb2_queue *q)
{
	...
	rc = wait_event_timeout(video->wait,
				...
				STOP_TIMEOUT);
	if (!rc) {
		v4l2_dbg(2, debug, &video->v4l2_dev, "Timed out when stopping streaming\n");
